...
inline vector<Ciphertext> PIRServer::expand_query(const Ciphertext &encrypted,
  ...
  //cout << "First loop of expand_query" << endl;
  ...
  for (uint32_t i = 0; i < logm - 1; i++) {
    ...
    int index_raw_mod = index_raw % (2 * n);
    if (index_raw_mod >= n) {
        index_raw_mod -= n;
    }
    ...
    for (uint32_t a = 0; a < temp.size(); a++) {
      ...
      multiply_power_of_X_correct(temp[a], tempctxt_shifted, index_raw);
      //multiply_power_of_X(temp[a], tempctxt_shifted, index_raw_mod);

      // cout << "mul by x^pow: " <<
      // client.decryptor_->invariant_noise_budget(tempctxt_shifted) << ", ";
      //cout << "edited" << endl;
      multiply_power_of_X_correct(tempctxt_rotated, tempctxt_rotatedshifted, index);
      //multiply_power_of_X(tempctxt_rotated, tempctxt_rotatedshifted, index_mod);
  ...
  //cout << "last loop of expand_query" << endl;
  // Last step of the loop
  vector<Ciphertext> newtemp(temp.size() << 1);
  int index_raw = (n << 1) - (1 << (logm - 1));
  uint32_t index_raw_mod = index_raw % (2 * n);
  if (index_raw_mod >= n) {
      index_raw_mod -= n;
  }
  int index = (index_raw * galois_elts[logm - 1]) % (n << 1);
  uint32_t index_mod = index % (2 * n);
  if (index_mod >= n) {
      index_mod -= n;
  }
  for (uint32_t a = 0; a < temp.size(); a++) {
...
inline void PIRServer::multiply_power_of_X_correct(
    const Ciphertext& encrypted,
    Ciphertext& destination,
    uint32_t index)
{
    auto n = enc_params_.poly_modulus_degree(); //n

    index %= (2 * n);  // keep PIR 2n semantics

    if (index < n)
    {
        // normal rotation
        multiply_power_of_X(encrypted, destination, index);
    }
    else
    {
        // wrap around negacyclic boundary
        uint64_t shifted = index - n;

        multiply_power_of_X(encrypted,destination, shifted);
        evaluator_->negate_inplace(destination);
    }
}
...
inline void PIRServer::multiply_power_of_X(const Ciphertext &encrypted,
  ...
  auto coeff_count = enc_params_.poly_modulus_degree(); //n
  ...
  for (int i = 0; i < encrypted_count; i++) {
    for (int j = 0; j < coeff_mod_count; j++) {
        /*cout << "calling negacylclic_shift_poly_coeffmod!!! " << endl;
        cout << "shift = " << index << endl;
