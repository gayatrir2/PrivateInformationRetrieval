...
mpz_t *DbGen_FromFile(int m, int n, mpz_t p, const char *filename) {
    mpz_t *X_db = (mpz_t *) malloc((m * n) * sizeof(mpz_t));

    // Read entire binary file: n patients * 128 floats each
    float *raw = (float *) malloc(n * 128 * sizeof(float));
    FILE *f = fopen(filename, "rb");
    if (!f) {
        printf("ERROR: Cannot open %s\n", filename);
        exit(1);
    }
    fread(raw, sizeof(float), n * 128, f);
    fclose(f);

    // X_db[i * m + j] = feature j of patient i
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            mpz_init(X_db[i * m + j]);
            float val = raw[i * 128 + j];
            unsigned long scaled = (unsigned long)(fabsf(val) * 1000000.0f);
            mpz_set_ui(X_db[i * m + j], scaled);
            mpz_mod(X_db[i * m + j], X_db[i * m + j], p);
        }
    }
    free(raw);
    return X_db;
