...
void print_patient(int i_index, int m, mpz_t *DataBase, const char *label) {
    char *features[] = {
        "scoma", "age", "meanbp", "wblc", "hrt", "resp",
        "temp", "pafi", "alb", "bili", "crea", "sod",
        "ph", "glucose", "bun", "urine", "adlsc",
        "dnr", "ca", "cancer", "pnum", "race",
        "income", "edu", "charges", "totcst"
    };
    printf("\n========= Patient x_%d [%s] =========\n", i_index, label);
    for (int j = 0; j < m; j++) {
        double val = mpz_get_d(DataBase[i_index * m + j]) / 1000000.0;
        printf("  %-10s : %.2f\n", features[j], val);
    }
    printf("==========================================\n\n");
}
void print_decoded(int i_index, int m, mpz_t *result, const char *label) {
    char *features[] = {
        "scoma", "age", "meanbp", "wblc", "hrt", "resp",
        "temp", "pafi", "alb", "bili", "crea", "sod",
        "ph", "glucose", "bun", "urine", "adlsc",
    ...
    printf("\n========= Patient x_%d [%s] =========\n", i_index, label);
    for (int j = 0; j < m; j++) {
        double val = mpz_get_d(result[j]) / 1000000.0;
        printf("  %-10s : %.2f\n", features[j], val);
    }
    printf("==========================================\n\n");
}
...
int main(int argc, char *argv[]) {
	int tamper_mode = (argc > 1 && strcmp(argv[1], "--tamper") == 0);
    	...
    	for (int n = 0; n < lengthn; n++)
	{
		...
		{	
			...
			start = clock();
			DataBase = DbGen_FromFile(m_value[m], n_value[n], p, "support2_cpir.bin");
			stop = clock();
			...
			start = clock();
    			...
    			mpz_t original_hash;
			mpz_init(original_hash);
			mpz_set(original_hash, hash_cols[i_index]);
    			
    			print_patient(i_index, m_value[m], DataBase, "ORIGINAL - Before Commitment");
			...
			start= clock();
			if (tamper_mode) {
			    double new_age, new_charges;

			    printf("\n[TAMPER MODE] Enter new age value (original: %.2f): ",
				mpz_get_d(DataBase[i_index * m_value[m] + 1]) / 1000000.0);
			    scanf("%lf", &new_age);

			    printf("[TAMPER MODE] Enter new charges value (original: %.2f): ",
				mpz_get_d(DataBase[i_index * m_value[m] + 24]) / 1000000.0);
			    scanf("%lf", &new_charges);

			    mpz_set_ui(DataBase[i_index * m_value[m] + 1],  (unsigned long)(new_age     * 1000000.0));
			    mpz_set_ui(DataBase[i_index * m_value[m] + 24], (unsigned long)(new_charges * 1000000.0));

			    printf("\n[TAMPER MODE] Age set to %.2f, Charges set to %.2f\n", new_age, new_charges);
			}
			...
			print_decoded(i_index, m_value[m], result, tamper_mode ? "DECODED - Tampered Data" : "DECODED - Honest Data");

			c_PIR_time += (double)(stop - start) / CLOCKS_PER_SEC;  
		
			// Recompute hash of the (possibly tampered) column
			mpz_t *hash_cols_after = HashGen_Server(m_value[m], n_value[n], DataBase, p);

			// Print both side by side
			printf("\n========= [INTEGRITY CHECK] Column x_%d Hash Comparison =========\n", i_index);
			gmp_printf("  BEFORE : %Zd\n", original_hash);
			gmp_printf("  AFTER  : %Zd\n", hash_cols_after[i_index]);
			if (mpz_cmp(original_hash, hash_cols_after[i_index]) == 0)
			    printf("  RESULT : MATCH ✓ — data integrity intact\n");
			else
			    printf("  RESULT : MISMATCH ✗ — tampering detected!\n");
			printf("==================================================================\n\n");

			mpz_clear(original_hash);
